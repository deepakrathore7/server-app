...
class DriverSignupController extends LoginController
{
    ...
    */
    ...
    public function register(DriverRegistrationRequest $request)
    {
        ...
        if(str_contains($request->country, '+')){
            $country_data = $this->country->where('active',true)->where('dial_code',$request->country)->first();
        }else{
            $country_data = $this->country->where('code',$request->country)->first();
        }
