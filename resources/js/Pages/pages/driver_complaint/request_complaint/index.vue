...
export default {
    ...
    setup(props) {
        ...
        const deleteData = async (dataId) => {
            try {
                await axios.delete(`/driver-complaint/driver-request-list/delete/${dataId}`);
                const index = results.value.findIndex(data => data.id === dataId);
                if (index !== -1) {
                    results.value.splice(index, 1);
                }
                modalShow.value = false;
                Swal.fire(t('success'), t('request_complaint_deleted_successfully'), 'success');
            } catch (error) {
                Swal.fire(t('error'), t('failed_to_delete_request_complaint'), 'error');
            }
        };
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/driver-complaint/driver-request-list`, { params });
                results.value = response.data.results;
                paginator.value = response.data.paginator;
                updatePaginatorOptions(paginator.value.total);// Update paginator options dynamically
                modalFilter.value = false;
            } catch (error) {
                console.error(t('error_fetching_request_complaint'), error);
            }
        };

        const handlePageChanged = async (page) => {
            localStorage.setItem("driver-complaint/driver-request-list", page); // Save to localStorage
            fetchDatas(page);
        };

        const updateStatus = async (result, status) => {
            try {
                // Pass `status` as query parameter using `params`
                const response = await axios.get(`/driver-complaint/taken/${result.id}`, {
                    params: {
                        status: status
                    }
                });

                // Set the success message when the status is successfully updated
                successMessage.value = t('status_updated_successfully');
                alertMessage.value = ""; // Clear any previous error messages

                // Optionally update the result object locally to reflect status change in UI
                result.status = status;
                
            } catch (error) {
                console.error(t('error_updating_status'), error);

                // Set the alert message in case of an error
                alertMessage.value = t('failed_to_update_status');
    ...
    mounted() {
        this.fetchDatas();
        const savedPage = localStorage.getItem("driver-complaint/driver-request-list");
        if(savedPage){
...
<template>
    <Layout>
        ...
        <Head title="General Complaint" />
        <PageHeader :title="$t('request_complaint')" :pageTitle="$t('request_complaint')" />
        <BRow>
            <BCol lg="12">
                        ...
                        <BRow class="g-2">
                            ...
                            <BCol md="auto" class="ms-auto">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <!-- <Link href="/complaint-title/create">
                                    <BButton variant="primary" class="float-end"> <i
                                            class="ri-add-line align-bottom me-1"></i> Add Complaint Title</BButton>
                                    </Link> -->
                                </div>
                            </BCol>
                        </BRow>
                                        ...
                                        <td>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu dropdown-menu-end">
                                                    <!-- Conditionally show "Taken" button if status is not "solved" -->
                                                    <a v-if="result.status !== 'solved'" class="dropdown-item" href="#" @click.prevent="updateStatus(result, 'taken')">
                                                    <i class="bx bxs-edit-alt align-center text-muted me-2"></i>{{ $t("taken") }}
                                                    </a>

                                                    <!-- Conditionally show "Solved" button if status is "taken" -->
                                                    <a v-if="result.status === 'taken'" class="dropdown-item" href="#" @click.prevent="updateStatus(result, 'solved')">
                                                    <i class="bx bxs-check-circle align-center text-muted me-2"></i>{{ $t("solved") }}
                                                    </a>
                                                </div>
                                            </div>
                                        </td>
