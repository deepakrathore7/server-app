...
export default {
    ...
    setup(props) {
        ...
        const deleteData = async (dataId) => {
            try {
                await axios.delete(`/driver-complaint/delete/${dataId}`);
                const index = results.value.findIndex(data => data.id === dataId);
                if (index !== -1) {
                    results.value.splice(index, 1);
                }
                modalShow.value = false;
                Swal.fire(t('success'), t('general_complaint_deleted_successfully'), 'success');
            } catch (error) {
                Swal.fire(t('error'), t('failed_to_delete_general_complaint'), 'error');
            }
        };
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/driver-complaint/list`, { params });
                ...
                console.error(t('error_fetching_general_complaint'), error);
            }
        };

        const handlePageChanged = async (page) => {
            localStorage.setItem("driver-complaint/list", page); // Save to localStorage
            fetchDatas(page);
        };
    ...
    mounted() {
        this.fetchDatas();
        const savedPage = localStorage.getItem("driver-complaint/list");
        if(savedPage){
...
<template>
    <Layout>
        ...
        <PageHeader :title="$t('general_complaint')" :pageTitle="$t('general_complaint')" />
        <BRow>
            <BCol lg="12">
