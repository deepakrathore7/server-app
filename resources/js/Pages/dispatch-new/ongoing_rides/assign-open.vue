...
const assignDriver = async (driver) => {
  try {
    const response = await axios.post(`/dispatcher/ongoing_request/assign-driver/${result.value.id}`,{ 'driver_id' : driver.id});
...
const fetchNearbyDrivers = () => {
    ...
    requestRef.on('value', (snapshot) => {
      ...
      if(val && (val.is_accept || val.hasOwnProperty('driver_id'))) {
          router.get(`/dispatcher/rides_request/view/${result.value.id}`);
      }
    });
                ...
                if (distance <= driver_radius.value && driver.is_available && driver.is_active && driver.vehicle_types.includes(result.value.vehicle_type_id)) {
                    ...
                    if (driverMarkers[driver.id]) {
                        ...
                        driverMarker.bindTooltip(
                            `<strong>${driver.name}</strong><br>Mobile: ${app_for == 'demo' ? '***********' : driver.mobile}`,
                            {
                                permanent: false,
                                direction: 'top',
                                offset: [0, -10],
...
watch(driverSearch.value,debounce(function(value){
  fetchNearbyDrivers();
},300));
...
<BCardHeader class="border-0">
  <div class="position-relative col-lg-4">
      <input type="text" class="form-control" placeholder="Search..." v-model="driverSearch" autocomplete="off" id="search-options" value="">
  </div>
</BCardHeader>
<BCardBody class="border border-dashed border-end-0 border-start-0">

<div class="row">
<div class="col-12 col-lg-5" style="overflow-x: auto;">
<div class="row">
          <BRow>
            <BCard no-body>
            ...
            <BCol xl="12">
                <div class="row" >
                  <!-- card -->
                        <div v-for="driver in fleetDrivers" :key="driver.driver_id" @click="assignModal(driver)" class="row">
                            ...
                            <div class="col-sm-6 d-flex align-items-center">
                              <div class="row">
                                <div class="col-lg-12">
                                  <div>
                                    <button type="button" class="btn btn-info btn-label waves-effect waves-light"><i class="ri-car-line label-icon fs-20 align-middle me-2"></i> {{ $t('assign') }}</button>
                                  </div>
                                </div>
                                <div class="col-lg-12">
