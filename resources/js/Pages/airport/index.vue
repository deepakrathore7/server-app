...
export default {
    ...
    setup(props) {
        ...
        //     try {
        //         await axios.post(`/airport/update-status`, { id: id, status: status });
        ...
        const toggleActiveStatus = async (id, status) => {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    try {
                        await axios.post(`/airport/update-status/${id}`, { id: id, status: status });
                        const index = results.value.findIndex(item => item.id === id);
                        if (index !== -1) {
                            results.value[index].active = status; // Update the active status locally
                        }
        ...
        const deleteData = async (dataId) => {
            try {
                await axios.delete(`/airport/delete/${dataId}`);
                const index = results.value.findIndex(data => data.id === dataId);
                if (index !== -1) {
                    results.value.splice(index, 1);
                }
                modalShow.value = false;
                Swal.fire(t('success'), t('airport_deleted_successfully'), 'success');
            } catch (error) {
                if (error.response && error.response.status == 403) {
                    alertMessage.value = error.response.data.alertMessage;
                    setTimeout(()=>{
                ...
                Swal.fire(t('error'), t('failed_to_delete_airport'), 'error');
            }
        };
        ...
        const fetchDatas = async (page = 1) => {
            try {
                ...
                params.page = page;
                const response = await axios.get(`/airport/fetch`, { params });
                ...
                console.error(t('error_fetching_airport'), error);
            }
        };

        const handlePageChanged = async (page) => {
            localStorage.setItem("airport/fetch", page); // Save to localStorage
            fetchDatas(page);
        };

        const editData = async (result) =>  {
            router.get(`/airport/edit/${result.id}`); 
        };

        const surgeData = async (result) =>  {
            router.get(`/airport/surge/${result.id}`); 
        };

        const mapData = async (result) =>  {
            router.get(`/airport/map/${result.id}`); 
        };
    ...
    mounted() {
        this.fetchDatas();
        const savedPage = localStorage.getItem("airport/fetch");
        if(savedPage){
...
<template>
    ...
    <Head title="Airport" />
    <PageHeader :title="$t('airport')" :pageTitle="$t('airport')" />
        <BRow>
            <BCol lg="12">
                        ...
                        <BRow class="g-2">
                            ...
                            <BCol md="auto" class="ms-auto">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    <Link href="/airport/create" v-if="permissions.includes('Add-Airport')">
                                    <BButton variant="primary" class="float-end" :disabled="app_for === 'demo'"> <i
                                            class="ri-add-line align-bottom me-1"></i>  {{ $t("add_airport") }}</BButton>
                                    </Link>
                                </div>
                            </BCol>
                        </BRow>
                            ...
                            <table class="table align-middle position-relative table-nowrap">
                                ...
                                <tbody v-if="results.length > 0">
                                    <tr v-for="(result, index) in results" :key="index">
                                        ...
                                        <td v-if="permissions.includes('Toggle-Airport')">
                                            <div :class="{
                                                    'form-check': true,
                                                    'form-switch': true,
                                                    'form-switch-lg': true,
                                        ...
                                        <td>
                                            <BButton class="btn btn-soft-warning btn-sm m-2" @click.prevent="editData(result)" v-if="permissions.includes('Edit-airport')"
                                                    data-bs-toggle="tooltip" v-b-tooltip.hover
                                                    :title="$t('edit')">
                                                    <i class='bx bxs-edit-alt bx-xs'></i>
                                            </BButton>
                                            <BButton class="btn btn-soft-info btn-sm m-2" size="sm"
                                                type="button" @click.prevent="mapData(result)" v-if="permissions.includes('Map-view-Airport')"
                                                data-bs-toggle="tooltip" v-b-tooltip.hover :title="$t('airport_map_view')">
                                                <i class=' ri-earth-line bx-xs'></i>
                                            </BButton>
                                            <BButton class="btn btn-soft-danger btn-sm m-2" size="sm"
                                                type="button" @click.prevent="deleteModal(result.id)" v-if="permissions.includes('Delete-Airport')"
        ...
        <!-- filter -->
        <!-- Filter Modal -->
        <BOffcanvas v-model="rightOffcanvas" placement="end" :title="$t('airport_filters')" header-class="bg-light"
